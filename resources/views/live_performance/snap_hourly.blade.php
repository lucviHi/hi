...
@section('content')
<div class="container">
    <h2 class="mb-4">📊 Tổng doanh thu của các kênh (theo ngày 123)</h2>
        ...
        <tbody>
            @forelse ($snapshot as $item)
                <tr>
                    ...
                    <td>{{ number_format($item->roas_total, 2) }}</td>
                    <td>{{ $item->views }}</td>
                    <td>{{ $item->product_clicks }}</td>
                    <td>{{ number_format($item->live_impressions) }}</td>
                    <td>{{ number_format($item->entry_rate, 2) }}</td>
                    <td>{{ number_format($item->ctr, 2) }}</td>
                    <td>{{ number_format($item->ctor, 2) }}</td>
                </tr>
            @empty
                <tr><td colspan="13">Không có dữ liệu cho ngày này.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
